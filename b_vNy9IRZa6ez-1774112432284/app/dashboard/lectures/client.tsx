...
export default function LecturesPage() {
  ...
  const handlePlayAudio = () => {
    ...
    setTimeout(() => {
      ...
      // Try to use browser speech synthesis for a real effect
      if ('speechSynthesis' in window) {
        const msg = new SpeechSynthesisUtterance("Welcome to the 3 minute AI audio over-view. Today we will cover LTI systems, Continuous-time Fourier transforms, and their real world applications in 5G networks.");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Tabs defaultValue="summary" className="w-full">
            ...
            <TabsContent value="realworld" className="mt-4 space-y-4">
              ...
              <Card className="bg-chart-5/5 border border-chart-5/20 mt-6">
                ...
                <CardContent>
                  ...
                  <Button variant="link" className="text-chart-5 p-0 h-auto mt-2 text-xs" onClick={() => toast({ title: "Lab Guide", description: "Navigating to lab simulation setup." })}>View Lab Guide →</Button>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="slides" className="mt-4">
               <Card className="bg-card/40 border-border/50 flex flex-col items-center justify-center p-12 py-20">
                 ...
                 <Button variant="outline" className="mt-4" onClick={() => toast({ title: "Downloading", description: "Your PDF is being generated..." })}>Download PDF (12MB)</Button>
               </Card>
            </TabsContent>
        ...
        <div className="space-y-6">
          <Card className="bg-card/40 border-border/50 backdrop-blur-md">
            ...
            <Card className="bg-gradient-to-br from-indigo-500/10 to-purple-600/20 border-2 border-indigo-500/50 backdrop-blur-md relative overflow-hidden group shadow-[0_0_20px_rgba(99,102,241,0.2)]">
              <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/20 rounded-bl-[100px] -z-10 transition-transform group-hover:scale-110" />
              <CardHeader>
                <CardTitle className="text-xl text-indigo-400 font-bold flex items-center gap-2">
                  <Volume2 className="w-5 h-5" /> Missed this class?
                </CardTitle>
                <CardDescription className="text-sm text-foreground/80 mt-2">
                  The AI can generate a rapid 3-minute audio digest summing up the exact points tested.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Button 
                  className={`w-full font-semibold shadow-lg transition-all ${
                    isGeneratingAudio ? 'bg-indigo-500/50 text-white animate-pulse' 
                    : isPlayingAudio ? 'bg-eureka-success/80 text-white' 
                ...
                >
                  {isGeneratingAudio ? (
                    ...
                    <><Volume2 className="w-4 h-4 mr-2 animate-pulse" /> Playing Audio Digest</>
                  ) : (
                    <><PlayCircle className="w-4 h-4 mr-2" /> Generate & Play Audio</>
                  )}
              </Button>
            </CardContent>
          </Card>
