...
const CustomNode = ({ data }: any) => {
  return (
    <div className={`px-4 py-3 shadow-[0_0_15px_rgba(0,0,0,0.5)] rounded-xl border object-contain backdrop-blur-md bg-black/60 w-[250px] cursor-pointer transition-all hover:scale-105 hover:bg-black/80 hover:shadow-[0_0_20px_rgba(100,50,250,0.6)] ${data.isRoot ? 'border-primary' : data.isModule ? 'border-blue-500' : 'border-orange-500/80'} ${data.isSelected ? 'ring-2 ring-primary ring-offset-2' : ''}`} onClick={() => data.onClick(data)}>
      <Handle type="target" position={Position.Top} className="w-16 !bg-primary" />
      <div className="flex items-center space-x-2">
        {data.isRoot && <BrainCircuit className="h-5 w-5 text-primary" />}
        {data.isModule && <Sparkles className="h-4 w-4 text-blue-500" />}
        {!data.isRoot && !data.isModule && <Variable className="h-4 w-4 text-orange-500" />}
      ...
      {data.isModule && (
        <div className="text-xs text-blue-200/70 mt-1 cursor-pointer hover:text-primary transition-colors">
    ...
    if (isOpen && subject && !roadmapData) {
      const fetchRoadmap = async () => {
        ...
        try {
          ...
          const res = await fetch(`${API_URL}/exam-prep/roadmap?subject=${encodeURIComponent(subject)}`)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`flex flex-col p-4 bg-black/80 border-none backdrop-blur-md shadow-[0_0_50px_rgba(100,50,255,0.4)] text-white overflow-hidden transition-all duration-300 ${isFullScreen ? '!max-w-none !w-screen !h-screen !m-0 !p-6 !rounded-none data-[state=open]:!zoom-in-100' : '!max-w-[95vw] !w-[95vw] !h-[95vh]'}`}>
        ...
        <div className="absolute inset-0 pointer-events-none z-[-1] overflow-hidden opacity-70">
          <CosmicBackground />
          <div className="absolute top-[20%] left-[80%] w-64 h-64 rounded-full bg-gradient-to-tr from-amber-600 to-red-900 blur-2xl opacity-60 mix-blend-screen animate-pulse"></div>
          <div className="absolute top-[30%] left-[80%] w-72 h-4 bg-orange-300 blur-md transform -rotate-12 opacity-80"></div>
          <div className="absolute top-[30%] left-[80%] w-80 h-1 bg-amber-100 transform -rotate-12 opacity-50"></div>
        </div>
        ...
        <DialogHeader className="mb-2 shrink-0 flex flex-row items-center justify-between">
          <div>
            <DialogTitle className="flex flex-row items-center gap-2 text-2xl text-white">
              <Maximize2 className="h-6 w-6 text-indigo-400" /> Interactive Study Roadmap: <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">{subject}</span>
            </DialogTitle>
          ...
          {loading && (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/60 backdrop-blur-md">
              <div className="relative w-32 h-32 mb-4 animate-[spin_10s_linear_infinite]">
                 ...
                 <div className="absolute inset-4 rounded-full bg-gradient-to-tr from-amber-500 to-orange-800 shadow-[0_0_30px_rgba(255,150,50,0.5)]"></div>
            ...
            <div className="w-[450px] h-full border-l border-white/20 bg-black/60 backdrop-blur-xl flex flex-col shrink-0 overflow-hidden shadow-2xl z-10 transition-all">
              ...
              <div className="p-4 grid grid-cols-2 gap-3 border-b border-white/10">
                <Button 
                  ...
                  className={`w-full text-xs font-semibold ${selectedPanel === 'explain' ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-white/10 hover:bg-white/20 text-gray-200'}`}
                >
