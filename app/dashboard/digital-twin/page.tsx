          ...
          <Card className="bg-card/40 border-border/50 backdrop-blur-md">
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <Progress value={digitalTwin.cognitiveProfile.visualLearning} className="h-1.5 bg-accent/20" />
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {digitalTwin.knowledgeNodes.strong.map(node => (
                      <Badge key={node} variant="secondary" className="bg-eureka-success/10 text-eureka-success font-normal border border-eureka-success/20 py-1">{node}</Badge>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {digitalTwin.knowledgeNodes.developing.map(node => (
                      <Badge key={node} variant="secondary" className="bg-accent/10 text-accent font-normal border border-accent/20 py-1">{node}</Badge>
                    ))}
                  </div>
                </div>

                <div>
                  <h4 className="text-xs text-muted-foreground uppercase tracking-wider mb-3">Weak Links (Critical Gaps)</h4>
                  <div className="flex flex-wrap gap-2">
                    {digitalTwin.knowledgeNodes.weak.map(node => (
                      <Badge key={node} variant="destructive" className="bg-destructive/10 text-destructive font-normal border border-destructive/20 py-1 flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" /> {node}
                      </Badge>
                    ))}
                  </div>
                </div>
